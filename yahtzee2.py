'''
Title: Yahtzee Version 2 Code: Lower Section - Lab 6 Demo
...
def num_of_a_kind(roll: tuple, number: int) -> int:
    ...
    Otherwise, returns 0.
    """
    sum_of_values = sum(roll)
    for i in range(len(roll)):
        rolls_count = roll.count(roll[i])
        if rolls_count == number:
            return sum_of_values
    return 0
...
def main():
    ...
    print("Lower Section:")
    print(f"Three of a kind: {num_of_a_kind(atuple, 3)}")
    print(f"Four of a kind: {num_of_a_kind(atuple, 4)}")
    print(f"Yahtzee: {yahtzee(atuple)}")
