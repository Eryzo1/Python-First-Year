"""
Author: Sami
Refrences: Lecture slides and peers
"""
...
class BinaryTree:
    ...
    def _strHelper(self):
        ...
        if self.getLeft() != None and self.getRight() == None:
            ...
            firstRow = (
                (leftMiddle + 1) * " " + (leftWidth - leftMiddle - 1) * "_" + keyStr
            )
            secondRow = (
                leftMiddle * " "
                + "/"
            ...
            return (
                [firstRow, secondRow] + shiftedRows,
                leftWidth + keyStrLength,
                leftWidth + keyStrLength // 2,
                leftHeight + 2,
            )

        # Case 2: only have right child
        elif self.getLeft() == None and self.getRight() != None:
            rightRows, rightWidth, rightMiddle, rightHeight = (
                self.getRight()._strHelper()
            )
            firstRow = keyStr + rightMiddle * "_" + (rightWidth - rightMiddle) * " "
            secondRow = (
                (keyStrLength + rightMiddle) * " "
                + "\\"
        ...
        # Two children.
            ...
            return (
                rows,
                leftWidth + rightWidth + keyStrLength,
                leftWidth + keyStrLength // 2,
...
def findMinKey(tree):
    ...
    if tree.getRight() is None:
        return tree.getKey()
    left_tree = findMinKey(tree.getLeft())
    right_tree = findMinKey(tree.getRight())
    return min(left_tree, right_tree, tree.getKey())
...
def findMaxKey(tree):
    ...
    if tree.getRight() is None:
        return tree.getKey()
    left_tree = findMaxKey(tree.getLeft())
    right_tree = findMaxKey(tree.getRight())
    return max(left_tree, right_tree, tree.getKey())
